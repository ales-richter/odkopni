...
const PLANT_DB = CZ_PERENNIALS.map(function(e) { var parts = e.split(" – "); return { latin: parts[0].trim(), czech: parts[1] ? parts[1].trim() : "", full: e }; });
...
function searchLocalPlants(q) {
  ...
  for (var i = 0; i < PLANT_DB.length; i++) {
    var p = PLANT_DB[i];
    var l = p.latin.toLowerCase();
    var c = p.czech.toLowerCase();
...
var STATUSES = {
  active: { label: "Dostupné", color: "#48a868", bg: "#e8f5ec" },
  reserved: { label: "Zamluvené", color: "#e8a040", bg: "#fff8e8" },
  traded: { label: "Vyměněno", color: "#999", bg: "#f0f0f0" }
};
...
function PlantAC({ value, onChange, placeholder }) {
  ...
  function doSearch(q) { setSugg(searchLocalPlants(q)); }
  ...
  function handleChange(e) {
    ...
    if (v.length >= 2) {
      timer.current = setTimeout(function() { doSearch(v); }, 150);
      setShow(true);
    } else {
      setSugg([]);
      setShow(false);
  ...
  useEffect(function() {
    function h(e) { if (wrap.current && !wrap.current.contains(e.target)) setShow(false); }
  ...
  return (
    <div ref={wrap} style={{ position: "relative" }}>
      <input value={value} onChange={handleChange} placeholder={placeholder}
        onFocus={function() { setFocused(true); if (sugg.length) setShow(true); }}
      ...
      {show && sugg.length > 0 && (
        <div style={{ position:"absolute", top:"100%", left:0, right:0, zIndex:50, background:"white", borderRadius:"12px", marginTop:"4px", boxShadow:"0 8px 32px rgba(0,0,0,0.12)", border:"1px solid " + C.border, maxHeight:"220px", overflow:"auto" }}>
          {sugg.map(function(s, i) {
            return (
              <div key={i} onMouseDown={function() { pick(s); }}
                style={{ padding:"10px 14px", cursor:"pointer", borderBottom: i < sugg.length - 1 ? "1px solid #f0f0f0" : "none" }}
                onMouseEnter={function(e) { e.currentTarget.style.background = "#f5f9f6"; }}
...
function Photos({ photos, onChange }) {
  ...
  function proc(file) {
    ...
    rd.onload = function(e) {
      ...
      img.onload = function() {
        ...
        onChange(photos.concat([cv.toDataURL("image/jpeg", 0.8)]));
      };
  ...
  return (
    <div>
      <label style={{ display:"block", fontSize:"13px", fontWeight:"600", color:C.sub, marginBottom:"8px" }}>
        Fotky ({photos.length}/{MAX})
      </label>
        ...
        {photos.map(function(p, i) {
          return (
            <div key={i} style={{ width:"80px", height:"80px", borderRadius:"12px", overflow:"hidden", position:"relative", boxShadow:"0 2px 8px rgba(0,0,0,0.1)" }}>
              ...
              <button onClick={function() { onChange(photos.filter(function(_, j) { return j !== i; })); }}
                style={{ position:"absolute", top:"3px", right:"3px", background:"rgba(0,0,0,0.6)", border:"none", borderRadius:"50%", width:"20px", height:"20px", color:"white", cursor:"pointer", fontSize:"11px", padding:0, display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
            </div>
          );
        })}
        {photos.length < MAX && (
          <>
            <button onClick={function() { camRef.current && camRef.current.click(); }}
              style={{ width:"80px", height:"80px", borderRadius:"12px", border:"2px dashed " + C.border, background:C.primary + "06", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"3px" }}>
              <I.Camera s={18} c={C.primary} /><span style={{ fontSize:"9px", color:C.primary, fontWeight:"600" }}>Fotit</span>
            </button>
            <button onClick={function() { galRef.current && galRef.current.click(); }}
              style={{ width:"80px", height:"80px", borderRadius:"12px", border:"2px dashed " + C.border, background:C.accent + "06", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"3px" }}>
              <I.Image s={18} c={C.accent} /><span style={{ fontSize:"9px", color:C.accent, fontWeight:"600" }}>Galerie</span>
            </button>
            <input ref={camRef} type="file" accept="image/*" capture="environment" style={{ display:"none" }}
              onChange={function(e) { if (e.target.files[0]) proc(e.target.files[0]); e.target.value = ""; }} />
...
function PlantCard({ plant, onClick, isFav, onToggleFav }) {
  var hasP = plant.photos && plant.photos.length > 0;
  var dn = (plant.name || "").split(" – ");
  var st = STATUSES[plant.status] || STATUSES.active;
  var isDemand = plant.type === "demand";
  return (
    <div onClick={onClick}
      style={{ background:C.card, borderRadius:"16px", overflow:"hidden", cursor:"pointer", transition:"all 0.3s cubic-bezier(0.23,1,0.32,1)", boxShadow:"0 2px 12px rgba(0,0,0,0.05)", border:"1px solid " + (isDemand ? C.accent + "40" : C.border) }}
      onMouseEnter={function(e) { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 8px 28px rgba(0,0,0,0.1)"; }}
      onMouseLeave={function(e) { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 2px 12px rgba(0,0,0,0.05)"; }}>
      <div style={{ height:"140px", display:"flex", alignItems:"center", justifyContent:"center", position:"relative", overflow:"hidden", background: hasP ? "#eee" : "linear-gradient(135deg, " + (plant.color || C.primary) + "20, " + (plant.color || C.primary) + "40)" }}>
        ...
        <button onClick={function(e) { e.stopPropagation(); onToggleFav(plant.id); }}
          style={{ position:"absolute", bottom:"8px", left:"8px", background:"rgba(255,255,255,0.85)", border:"none", borderRadius:"50%", width:"32px", height:"32px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(4px)", boxShadow:"0 1px 4px rgba(0,0,0,0.1)" }}>
      ...
      <div style={{ padding:"12px 14px" }}>
        ...
        {plant.lookingFor && (
          <div style={{ marginTop:"8px", padding:"6px 10px", background:C.primary + "08", borderRadius:"8px", fontSize:"11px", color:C.sub }}>
...
function GalleryLB({ photos, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.92)", zIndex:2000, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }} onClick={onClose}>
      ...
      <img src={photos[cur]} alt="" style={{ maxWidth:"92%", maxHeight:"78vh", borderRadius:"8px", objectFit:"contain" }} onClick={function(e) { e.stopPropagation(); }} />
      {photos.length > 1 && (
        <div style={{ display:"flex", gap:"8px", marginTop:"16px" }} onClick={function(e) { e.stopPropagation(); }}>
          {photos.map(function(p, i) {
            return <div key={i} onClick={function() { setCur(i); }} style={{ width:"48px", height:"48px", borderRadius:"8px", overflow:"hidden", cursor:"pointer", border: i === cur ? "2px solid white" : "2px solid transparent", opacity: i === cur ? 1 : 0.5 }}><img src={p} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} /></div>;
          })}
        </div>
      )}
    </div>
...
function Detail({ plant, user, onClose, onStartChat, onDelete, onStatusChange, isFav, onToggleFav }) {
  ...
  var mine = user && plant.userId === user.uid;
  var hasP = plant.photos && plant.photos.length > 0;
  var dn = (plant.name || "").split(" – ");
  var st = STATUSES[plant.status] || STATUSES.active;
  var isDemand = plant.type === "demand";
      ...
      <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.5)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"16px" }} onClick={onClose}>
        <div style={{ background:C.card, borderRadius:"24px", maxWidth:"480px", width:"100%", maxHeight:"88vh", overflow:"auto", boxShadow:"0 24px 80px rgba(0,0,0,0.2)" }} onClick={function(e) { e.stopPropagation(); }}>
          <div style={{ height:"200px", position:"relative", borderRadius:"24px 24px 0 0", overflow:"hidden", cursor: hasP ? "pointer" : "default", background: hasP ? "#eee" : "linear-gradient(135deg, " + (plant.color || C.primary) + "30, " + (plant.color || C.primary) + "60)", display:"flex", alignItems:"center", justifyContent:"center" }} onClick={function() { if (hasP) setGal(true); }}>
            ...
            <button onClick={function(e) { e.stopPropagation(); onToggleFav(plant.id); }} style={{ position:"absolute", top:"12px", left:"12px", background:"rgba(255,255,255,0.9)", border:"none", borderRadius:"50%", width:"36px", height:"36px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
          ...
          <div style={{ padding:"24px" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"16px" }}>
              ...
              {plant.lookingFor && (
                <div style={{ background:C.primary + "08", padding:"14px", borderRadius:"12px" }}>
                ...
                {!isDemand && (
                  <div style={{ display:"flex", gap:"6px" }}>
                    {Object.entries(STATUSES).map(function(entry) {
                      ...
                      return <button key={key} onClick={function() { onStatusChange(plant.id, key); }} style={{ flex:1, padding:"10px", border:"2px solid", borderColor: plant.status === key ? val.color : C.border, borderRadius:"10px", background: plant.status === key ? val.bg : "white", color: plant.status === key ? val.color : C.muted, fontSize:"12px", fontWeight:"600", cursor:"pointer" }}>{val.label}</button>;
                    })}
                  </div>
                )}
...
// ── Add / Demand Form ──
function AddForm({ user, profile, onClose, onAdd, mode }) {
  var isDemand = mode === "demand";
  const [f, setF] = useState({ name:"", category:"Kvetoucí", description:"", lookingFor:"", location: (profile && profile.location) || "" });
  ...
  async function submit() {
    ...
    try {
      var plantData = {
        ...
        photos: photos, type: mode, userId: user.uid,
        userName: (profile && profile.displayName) || "Anonym",
      ...
      alert("Nepodařilo se uložit. Zkuste to znovu.");
      setSaving(false);
    }
  }
      ...
      <div style={{ background:C.card, borderRadius:"24px", maxWidth:"480px", width:"100%", maxHeight:"90vh", overflow:"auto", boxShadow:"0 24px 80px rgba(0,0,0,0.2)" }} onClick={function(e) { e.stopPropagation(); }}>
        <div style={{ padding:"24px" }}>
          ...
          <div style={{ marginBottom:"14px" }}>
            ...
            <PlantAC value={f.name} onChange={function(v) { setF(Object.assign({}, f, { name: v })); }} placeholder={isDemand ? "např. Pivoňka, Hosta..." : "např. Echinacea, Třapatka..."} />
          </div>
          <div style={{ marginBottom:"14px" }}>
            <label style={{ display:"block", fontSize:"13px", fontWeight:"600", color:C.sub, marginBottom:"6px" }}>{isDemand ? "Podrobnosti *" : "Popis *"}</label>
            <textarea value={f.description} onChange={function(e) { setF(Object.assign({}, f, { description: e.target.value })); }}
              placeholder={isDemand ? "Jakou barvu, velikost hledáte..." : "Popište rostlinu, kolik kusů nabízíte..."} rows={3}
              ...
              onFocus={function(e) { e.target.style.borderColor = C.primary; }}
          ...
          {!isDemand && (
            <div style={{ marginBottom:"14px" }}>
              ...
              <input value={f.lookingFor} onChange={function(e) { setF(Object.assign({}, f, { lookingFor: e.target.value })); }}
                placeholder="Co byste chtěli výměnou?"
          ...
          <div style={{ marginBottom:"14px" }}>
            ...
            <input value={f.location} onChange={function(e) { setF(Object.assign({}, f, { location: e.target.value })); }}
              placeholder="Město"
              style={{ width:"100%", padding:"11px 14px", border:"2px solid " + C.border, borderRadius:"12px", fontSize:"14px", outline:"none", boxSizing:"border-box" }}
              onFocus={function(e) { e.target.style.borderColor = C.primary; }}
              onBlur={function(e) { e.target.style.borderColor = C.border; }} />
          ...
          {!isDemand && (
            <div style={{ marginBottom:"20px" }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:"6px" }}>
                {CATS.filter(function(c) { return c !== "Vše"; }).map(function(cat) {
                  return <button key={cat} onClick={function() { setF(Object.assign({}, f, { category: cat })); }}
                    style={{ padding:"6px 12px", borderRadius:"20px", border:"2px solid", borderColor: f.category === cat ? C.primary : C.border, background: f.category === cat ? C.primary + "10" : "white", color: f.category === cat ? C.primary : C.muted, fontSize:"12px", fontWeight:"600", cursor:"pointer" }}>{cat}</button>;
                })}
              </div>
            </div>
          )}
          <button onClick={submit} disabled={!ok || saving}
            style={{ width:"100%", padding:"14px", border:"none", borderRadius:"14px", background: ok ? "linear-gradient(135deg, " + C.primary + ", " + C.primaryDark + ")" : "#e0e0e0", color:"white", fontSize:"15px", fontWeight:"700", cursor: ok ? "pointer" : "default", opacity: saving ? 0.7 : 1 }}>
...
// ── Chat View (iOS-safe, solid background) ──
function ChatView({ chat, user, onBack, onMarkRead }) {
  ...
  useEffect(function() {
    var q2 = query(collection(db, "messages"), where("chatId", "==", chat.id), orderBy("timestamp", "asc"));
    var unsub = onSnapshot(q2, function(snap) {
  ...
  useEffect(function() {
    setTimeout(function() { if (btm.current) btm.current.scrollIntoView({ behavior: "smooth" }); }, 100);
  }, [msgs]);

  // Mark as read when opening chat
  useEffect(function() {
    if (onMarkRead) onMarkRead(chat.id);
  }, [chat.id]);
  ...
  async function send() {
    ...
    var chatRef = doc(db, "chats", chat.id);
    var chatSnap = await getDoc(chatRef);
    if (chatSnap.exists()) {
      await updateDoc(chatRef, { lastMessage: t, lastTimestamp: Date.now(), lastSenderId: user.uid, readBy: [user.uid] });
    } else {
      await setDoc(chatRef, { id: chat.id, participants: chat.participants, plantName: chat.plantName, lastMessage: t, lastTimestamp: Date.now(), lastSenderId: user.uid, readBy: [user.uid] });
    }
  }
  ...
  return (
    <>
      {/* Solid background layer that covers everything */}
      <div style={{ position:"fixed", top:0, left:0, width:"100vw", height:"100vh", backgroundColor:C.bg, zIndex:999 }} />
      <div ref={containerRef} style={{ position:"fixed", top:0, left:0, width:"100vw", zIndex:1000, display:"flex", flexDirection:"column", height:"100vh", height:"100dvh", backgroundColor:C.card, overflow:"hidden" }}>
        {/* Header */}
        <div style={{ padding:"12px 16px", borderBottom:"1px solid " + C.border, display:"flex", alignItems:"center", gap:"12px", flexShrink:0, backgroundColor:C.card }}>
          ...
          <div>
            <div style={{ fontSize:"15px", fontWeight:"700", color:C.text }}>{chat.plantName}</div>
            <div style={{ fontSize:"12px", color:C.muted }}>Výměna</div>
          </div>
        </div>
        {/* Messages */}
        ...
        <div style={{ padding:"8px 12px", paddingBottom:"calc(8px + env(safe-area-inset-bottom, 0px))", borderTop:"1px solid " + C.border, display:"flex", gap:"8px", alignItems:"center", boxSizing:"border-box", width:"100%", flexShrink:0, backgroundColor:C.card }}>
          <input value={text}
            ...
            onFocus={function() { setTimeout(function() { if (btm.current) btm.current.scrollIntoView({ behavior: "smooth" }); }, 300); }}
          ...
          <button onClick={send} disabled={!text.trim()}
            style={{ width:"40px", height:"40px", minWidth:"40px", borderRadius:"50%", border:"none", cursor: text.trim() ? "pointer" : "default", background: text.trim() ? C.primary : C.border, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            <I.Send s={16} c="white" />
          </button>
        </div>
      </div>
...
function ChatList({ user, onOpen, onBack }) {
  ...
  useEffect(function() {
    (async function() {
      ...
      setChats(snap.docs.map(function(d) { return Object.assign({ id: d.id }, d.data()); }));
      setLd(false);
    })();
  }, [user]);
    ...
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, backgroundColor:C.card, zIndex:900, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, overflow:"auto" }}>
        ...
        {chats.map(function(ch) {
          return (
            <div key={ch.id} onClick={function() { onOpen(ch); }}
              style={{ padding:"16px 20px", borderBottom:"1px solid " + C.border, cursor:"pointer", display:"flex", alignItems:"center", gap:"14px" }}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:"14px", fontWeight:"600", color:C.text }}>{ch.plantName}</div>
                <div style={{ fontSize:"12px", color:C.muted, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{ch.lastMessage}</div>
              </div>
            </div>
          );
        })}
    ...
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"linear-gradient(160deg, " + C.bg + " 0%, #e8f0ea 50%, " + C.bg + " 100%)", padding:"24px" }}>
      <div style={{ width:"100%", maxWidth:"380px", textAlign:"center" }}>
        ...
        <div style={{ textAlign:"left", marginBottom:"14px" }}>
          ...
          <input value={name} onChange={function(e) { setName(e.target.value); }} placeholder="Přezdívka nebo jméno"
        ...
        <div style={{ textAlign:"left", marginBottom:"24px" }}>
          ...
          <input value={loc} onChange={function(e) { setLoc(e.target.value); }} placeholder="Město nebo oblast"
            style={{ width:"100%", padding:"14px", border:"2px solid " + C.border, borderRadius:"14px", fontSize:"15px", outline:"none", boxSizing:"border-box" }}
            onFocus={function(e) { e.target.style.borderColor = C.primary; }}
            onBlur={function(e) { e.target.style.borderColor = C.border; }} />
        </div>
        <button onClick={go} disabled={!ok || saving}
          style={{ width:"100%", padding:"16px", border:"none", borderRadius:"14px", background: ok ? "linear-gradient(135deg, " + C.primary + ", " + C.primaryDark + ")" : "#e0e0e0", color:"white", fontSize:"16px", fontWeight:"700", cursor: ok ? "pointer" : "default", boxShadow: ok ? "0 4px 20px " + C.primary + "30" : "none" }}>
...
function ProfileEdit({ user, profile, onClose, onProfileUpdate }) {
  ...
  async function save() {
    if (!name.trim()) return;
    var data = { displayName: name.trim(), location: loc.trim() };
    ...
    <div style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.5)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:"16px" }} onClick={onClose}>
      <div style={{ background:C.card, borderRadius:"24px", maxWidth:"400px", width:"100%", boxShadow:"0 24px 80px rgba(0,0,0,0.2)" }} onClick={function(e) { e.stopPropagation(); }}>
        <div style={{ padding:"24px" }}>
          ...
          <div style={{ marginBottom:"20px" }}>
            <label style={{ display:"block", fontSize:"13px", fontWeight:"600", color:C.sub, marginBottom:"6px" }}>Lokalita</label>
            <input value={loc} onChange={function(e) { setLoc(e.target.value); }}
              style={{ width:"100%", padding:"11px 14px", border:"2px solid " + C.border, borderRadius:"12px", fontSize:"14px", outline:"none", boxSizing:"border-box" }}
              onFocus={function(e) { e.target.style.borderColor = C.primary; }}
              onBlur={function(e) { e.target.style.borderColor = C.border; }} />
          </div>
...
export default function App() {
  ...
  const [favs, setFavs] = useState(function() {
    try { return JSON.parse(localStorage.getItem("odkopni_favs") || "[]"); } catch (e) { return []; }
  });
  ...
  useEffect(function() {
    var unsub = onAuthStateChanged(auth, async function(u) {
      ...
      if (u) {
        try {
          var profSnap = await getDoc(doc(db, "profiles", u.uid));
  ...
  useEffect(function() {
    (async function() {
      try {
        ...
        var snap = await getDocs(q2);
        setPlants(snap.docs.map(function(d) { return Object.assign({ id: d.id }, d.data()); }));
    ...
    async function checkUnread() {
      try {
        ...
        snap.docs.forEach(function(d) {
          ...
          if (data.lastSenderId && data.lastSenderId !== user.uid && !readBy.includes(user.uid)) {
            count++;
          }
        });
  ...
  // Save favs
  useEffect(function() {
    try { localStorage.setItem("odkopni_favs", JSON.stringify(favs)); } catch (e) {}
  }, [favs]);

  function toggleFav(id) {
    setFavs(function(f) { return f.includes(id) ? f.filter(function(x) { return x !== id; }) : f.concat([id]); });
  }

  function addPlant(p) { setPlants(function(prev) { return [p].concat(prev); }); }

  async function delPlant(id) {
    try { await deleteDoc(doc(db, "plants", id)); setPlants(plants.filter(function(p) { return p.id !== id; })); }
    catch (e) { console.error(e); }
  }

  async function changeStatus(id, status) {
    try {
      await updateDoc(doc(db, "plants", id), { status: status });
      setPlants(plants.map(function(p) { return p.id === id ? Object.assign({}, p, { status: status }) : p; }));
      setSel(function(s) { return s && s.id === id ? Object.assign({}, s, { status: status }) : s; });
    } catch (e) { console.error(e); }
  }

  function startChat(plant) {
    var ids = [user.uid, plant.userId].sort();
    var chatId = ids[0] + "_" + ids[1] + "_" + plant.id;
    setActiveChat({ id: chatId, plantName: plant.name, participants: [user.uid, plant.userId] });
  }

  // Mark chat as read — adds current user to readBy array
  async function markRead(chatId) {
    try {
      var chatRef = doc(db, "chats", chatId);
      var chatSnap = await getDoc(chatRef);
      if (chatSnap.exists()) {
        var data = chatSnap.data();
        var readBy = data.readBy || [];
        if (!readBy.includes(user.uid)) {
          readBy.push(user.uid);
          await updateDoc(chatRef, { readBy: readBy });
        }
      }
      // Recount unread
      var q2 = query(collection(db, "chats"), where("participants", "array-contains", user.uid));
      ...
      snap.docs.forEach(function(d) {
        var dd = d.data();
        var rb = dd.readBy || [];
        if (dd.lastSenderId && dd.lastSenderId !== user.uid && !rb.includes(user.uid)) count++;
      });
      setUnreadCount(count);
    } catch (e) {}
  }
  ...
  var seasonalMsg = SEASONAL[month];
  ...
  // Loading
  if (user === undefined) {
    return <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:C.bg }}><div style={{ fontSize:"36px" }}>🌱</div></div>;
  }
  ...
  // Loading profile
  if (!profile) {
    return <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:C.bg, flexDirection:"column", gap:"12px" }}><div style={{ fontSize:"36px" }}>🌱</div><div style={{ color:C.muted, fontSize:"14px" }}>Načítám profil...</div></div>;
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans', 'Segoe UI', system-ui, sans-serif" }}>
      <style>{
        ...
        "*{box-sizing:border-box} input,textarea,button{font-family:inherit} html,body{overflow-x:hidden}"
      }</style>

      {/* Header - two rows for mobile */}
        ...
        <div style={{ maxWidth:"960px", margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"8px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
              ...
              <span style={{ fontSize:"18px", fontWeight:"800", fontFamily:"'Playfair Display', Georgia, serif", color:C.text }}>Odkopni</span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:"4px" }}>
              ...
              <button onClick={function() { setShowChats(true); }} style={{ background:"none", border:"none", cursor:"pointer", padding:"7px", borderRadius:"10px", position:"relative" }} title="Zprávy">
                <I.Chat s={19} c={C.sub} />
                {unreadCount > 0 && <span style={{ position:"absolute", top:"2px", right:"2px", background:C.heart, color:"white", borderRadius:"50%", width:"16px", height:"16px", fontSize:"10px", fontWeight:"700", display:"flex", alignItems:"center", justifyContent:"center" }}>{unreadCount}</span>}
              </button>
            </div>
          </div>
          <div style={{ display:"flex", gap:"8px" }}>
            <button onClick={function() { setAddMode("offer"); setShowAdd(true); }}
              style={{ flex:1, background:C.primary, border:"none", borderRadius:"10px", padding:"9px 12px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"5px", color:"white", fontSize:"13px", fontWeight:"700" }}>
              <I.Plus s={14} c="white" /> Nabídka
            </button>
            <button onClick={function() { setAddMode("demand"); setShowAdd(true); }}
              style={{ flex:1, background:C.accent, border:"none", borderRadius:"10px", padding:"9px 12px", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"5px", color:"white", fontSize:"13px", fontWeight:"700" }}>
      ...
      {/* Seasonal banner */}
      {seasonalMsg && (
        <div style={{ background:"linear-gradient(135deg, " + C.primary + "12, " + C.accent + "12)", padding:"10px 20px", fontSize:"13px", color:C.sub, textAlign:"center", borderBottom:"1px solid " + C.border }}>
          🌿 {seasonalMsg}
        </div>
      )}
      ...
      <div style={{ maxWidth:"960px", margin:"0 auto", padding:"14px 20px 0" }}>
        ...
        <div style={{ display:"flex", gap:"4px", marginBottom:"12px", background:"white", borderRadius:"12px", padding:"3px", border:"1px solid " + C.border, width:"fit-content" }}>
          {[
            { k:"browse", l:"Procházet", ico: I.Home },
            { k:"my", l:"Moje", ico: I.Leaf },
            { k:"favs", l:"Oblíbené", ico: function(p) { return <I.Heart s={p.s} c={p.c} filled={true} />; } }
          ].map(function(item) {
            var Ico = item.ico;
            return (
              <button key={item.k} onClick={function() { setView(item.k); }}
                style={{ padding:"7px 14px", borderRadius:"10px", border:"none", background: view === item.k ? C.primary : "transparent", color: view === item.k ? "white" : C.sub, fontSize:"12px", fontWeight:"600", cursor:"pointer", display:"flex", alignItems:"center", gap:"5px" }}>
                <Ico s={13} c={view === item.k ? "white" : C.muted} /> {item.l}
                {item.k === "favs" && favs.length > 0 && <span style={{ background: view === item.k ? "rgba(255,255,255,0.3)" : C.heart, color:"white", borderRadius:"10px", padding:"1px 6px", fontSize:"10px", fontWeight:"700" }}>{favs.length}</span>}
              </button>
            );
          })}
        </div>
        ...
        <div style={{ display:"flex", gap:"8px", marginBottom:"12px" }}>
          <div style={{ position:"relative", flex:1 }}>
            ...
            <input value={search} onChange={function(e) { setSearch(e.target.value); }} placeholder="Hledat rostliny, lokality..."
          ...
          <button onClick={function() { setSort(function(s) { return s === "newest" ? "oldest" : "newest"; }); }}
            style={{ background:"white", border:"2px solid " + C.border, borderRadius:"12px", padding:"0 12px", cursor:"pointer", display:"flex", alignItems:"center", gap:"5px", fontSize:"12px", color:C.sub, fontWeight:"600", whiteSpace:"nowrap" }}>
          ...
          {CATS.map(function(c) {
            return (
              <button key={c} onClick={function() { setCat(c); }}
                style={{ padding:"6px 12px", borderRadius:"20px", background: cat === c ? C.primary : "white", color: cat === c ? "white" : C.sub, fontSize:"12px", fontWeight:"600", cursor:"pointer", whiteSpace:"nowrap", border: cat === c ? "none" : "1px solid " + C.border, boxShadow: cat === c ? "0 2px 8px " + C.primary + "30" : "none" }}>{c}</button>
            );
          })}
        </div>
        ...
        <div style={{ display:"flex", gap:"12px", fontSize:"11px", color:C.muted, marginBottom:"14px" }}>
          ...
          <span><strong style={{ color:C.primary }}>{new Set(plants.map(function(p) { return p.location; })).size}</strong> měst</span>
        </div>
      </div>
            ...
            {filtered.map(function(p, i) {
              return (
                <div key={p.id} style={{ animation:"slideUp 0.35s ease " + (i * 0.04) + "s both" }}>
                  <PlantCard plant={p} onClick={function() { setSel(p); }} isFav={favs.includes(p.id)} onToggleFav={toggleFav} />
                </div>
              );
            })}
          </div>
      ...
      {/* Modals */}
      {sel && <Detail plant={sel} user={user} onClose={function() { setSel(null); }} onStartChat={startChat} onDelete={delPlant} onStatusChange={changeStatus} isFav={favs.includes(sel.id)} onToggleFav={toggleFav} />}
      {showAdd && <AddForm user={user} profile={profile} onClose={function() { setShowAdd(false); }} onAdd={addPlant} mode={addMode} />}
      {showChats && !activeChat && <ChatList user={user} onOpen={setActiveChat} onBack={function() { setShowChats(false); }} />}
      {activeChat && <ChatView chat={activeChat} user={user} onBack={function() { setActiveChat(null); }} onMarkRead={markRead} />}
